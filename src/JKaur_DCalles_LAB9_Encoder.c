/**************************************************************************
* File Name             EncoderCmd.c
* Description           Initialization of GPIO pins for DC motor and 
*                       moving motor in forward or reverse direction to 
*                       reach specified position based on arguments.
*				          
* Date				          Name(s)						          Action
...
{ 
  ...
  if(fetch_int32_arg(&posx)){
    printf("No argument."
    "Please enter desired position of the DC motor\n");
  }
  else{
    if(posx > 0){
      // moves motor in forward direction for positive arguments
      HAL_GPIO_WritePin(DC_MOTOR_PORT, DC_MOTOR_IN1_PIN, GPIO_PIN_SET);
      HAL_GPIO_WritePin(DC_MOTOR_PORT, DC_MOTOR_IN2_PIN, GPIO_PIN_RESET);
      desiredPosition = posx;
    }
    else{
      // moves motor in backward direction for negative arguments
